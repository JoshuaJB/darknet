...
#ifdef PAIRED
__thread long long *_rt_start_time;
__thread long long *_rt_end_time;
#else
__thread float *_rt_exec_time;
#endif
#if MMDC_PERF
__thread float *_rt_mmdc_read;
__thread float *_rt_mmdc_write;
#endif
__thread long _rt_jobs_complete;
__thread long _rt_max_jobs;
__thread int _rt_core;
__thread int _rt_will_output;
__thread struct timespec _rt_start, _rt_end;

__thread char *_rt_run_id;
__thread char *_rt_our_prog_name;
__thread char *_rt_other_prog_name;
__thread char *_rt_other_core;
#define _RT_FILENAME_LEN 64
#define _BILLION (1000*1000*1000)
#ifdef PAIRED
__thread char *_rt_barrier;
__thread sem_t *_rt_first_sem, *_rt_second_sem;
__thread int _rt_lock_id;
#define _ID_SZ 128
__thread char _rt_sem1_name[_ID_SZ] = "/_libextra_first_sem-";
__thread char _rt_sem2_name[_ID_SZ] = "/_libextra_second_sem-";
__thread char _rt_shm_name[_ID_SZ] = "/_libextra_barrier-";
#endif /* PAIRED */

#if LITMUS
__thread lt_t _rt_period;
__thread int _rt_crit;
__thread struct control_page *_rt_cp;
#endif

static void _rt_load_params_itrl(int argc, char **argv) {
#ifdef PAIRED
    if (argc != (8 + LITMUS*2) && argc != (9 + LITMUS*2)) {
        fprintf(stderr, "Usage: %s <name> <loops> <my core> <other core> <other name> <runID> <save results?> <pairID>\n", argv[0]);
#else
    if (argc != (6 + LITMUS*2)) {
...
#if LITMUS
#if PAIRED
    // If we're the 2nd task, we can read the PID of our paired task in the last 4 bytes of _rt_barrier
    uint32_t *master_pid = ((uint32_t*)_rt_barrier + 1);
    if (_rt_lock_id == 1)
        *master_pid = gettid();
#endif /* PAIRED */
...
#ifdef PAIRED
    // We don't use this, so repurpose it to indicate our pair master
    rt_param.phase = _rt_lock_id == 2 ? *master_pid : 0;
#else
    rt_param.phase = 0;
#endif /* PAIRED */
